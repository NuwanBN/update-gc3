...
1. **Call `ordino_workflow()`** (no args) → you get the workflow map. Pick the entry **path** that
   matches the task:
...
2. **Call `ordino_workflow(node=..., from=...)`** to pull each phase's goal, steps, rules, tools, and
   transitions. Work the node, and when its done-condition is met, fetch the next node it points to —
   passing `from` so the graph can sanity-check the transition (it soft-warns, never blocks).

3. **The workflow is advisory, not a cage.** Follow it — explicit structure is what keeps generation
   accurate. But for a novel scenario the graph doesn't cover, adapt: compose phases, or fall back to
   editing `.ts` directly under the conventions.
